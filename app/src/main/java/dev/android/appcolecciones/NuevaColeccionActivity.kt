...
import kotlinx.android.synthetic.main.activity_nueva_coleccion.*
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class NuevaColeccionActivity : AppCompatActivity() {
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        inflater.inflate(R.menu.accion_guardar_coleccion, menu)
        return true
    }
    ...
    private fun registrarMenu(){
        ...
        val menu = dev.android.appcolecciones.clases.Coleccion(nombre, detalle, precio.toDouble(), R.drawable.placeholder)
        CoroutineScope(Dispatchers.IO).launch {
            val id = database.menus().insertarMenu(menu)[0]
            uriImagen?.let {
                ControladorImagen.guardarImagen(this@NuevaColeccionActivity,id,it)
            }
            this@NuevaColeccionActivity.finish()
        }
    }
}